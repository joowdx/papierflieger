...
<nav class="px-4 py-0 navbar bg-base-100">
    <div class="navbar-start">
        {#if user}
            <div class="dropdown">
                <label for="navigation-items" class="btn btn-accent lg:hidden">
                ...
                <ul id="navigation-items" class="p-2 mt-3 shadow menu menu-compact tabs tabs-boxed dropdown-content bg-base-100 rounded-box w-52">
                    {#each navigation as item}
                        <li class="w-full">
                            <Link href={item.href} class={item.current ? 'tab-active tab' : 'tab'}> { item.name } </Link>
                        </li>
                    {/each}
                </ul>
            </div>
        ...
        {#if user}
            <div class="items-end hidden h-full ml-5 lg:flex">
                <ul class="flex p-0">
                    {#each navigation as item}
                        <li>
                            <Link href={item.href} class={item.current ? 'border-accent tab tab-bordered p-0 px-3 text-base-content' : 'tab tab-bordered p-0 px-3 text-base-content'}> { item.name } </Link>
                        </li>
                    {/each}
                </ul>
            </div>
    ...
    <div class="navbar-end">
        ...
        {#if user}
            <div class="dropdown dropdown-end">
                ...
                <ul tabindex="0" class="p-2 mt-4 shadow menu menu-compact dropdown-content bg-base-100 rounded-box w-52">
                    {#each dropdown as item}
                        <li>
                            <button use:inertia={{href: item.href, method: item.method ?? 'GET'}} class="justify-between" type="button">
                                { item.name }
                                {#if item.badge}
            ...
            <!-- <div class="dropdown dropdown-end">
                <label for="dropdown-items" class="btn btn-ghost btn-circle avatar" >
                ...
                <ul id="dropdown-items" class="p-2 mt-3 shadow menu menu-compact dropdown-content bg-base-100 rounded-box w-52">
                    {#each dropdown as item}
                        <li>
                            <button use:inertia href={item.href} method={item.method ?? 'GET'} class="justify-between">
                                { item.name }
                                {#if item.badge}
                                    <span class="badge bg-accent">
                                        { item.badge }
        ...
        {:else if ! user && ! route().current('login')}
            <!-- svelte-ignore missing-declaration -->
            <Link as="button" href={route('login')} class="ml-4 normal-case btn btn-ghost">Sign In</Link>
        {/if}
    </div>
</nav>
